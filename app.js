...
export default function DegreePlanner() {
    ...
    // 4. Load Main Catalog (courses.csv)
    useEffect(() => {
    ...
    const updateConnections = useCallback(() => {
        ...
        requestAnimationFrame(() => {
            try {
                ...
                const baseLineColor = isDarkMode ? "#475569" : "#cbd5e1"; 

                // Show all prerequisite lines if toggled
                ...
                // Show specific lines for hovered course
                if (hoveredCourse) {
    ...
    const resetAllData = () => {
        if(window.confirm("בטוח שברצונך למחוק הכל ולהתחיל מחדש?")) {
